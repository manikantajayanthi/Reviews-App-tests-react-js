...
class App extends Component {
  ...
  onClickLeftChange = () => {
    ...
    if (selectedCompanyIndex > 0) {
      this.setState({selectedCompanyIndex: selectedCompanyIndex - 1})
    } else {
      this.setState({selectedCompanyIndex: 0})
    }
  }

  onClickRightChange = () => {
    const {selectedCompanyIndex} = this.state
    if (selectedCompanyIndex < reviewsList.length - 1) {
      this.setState({selectedCompanyIndex: selectedCompanyIndex + 1})
    } else {
      this.setState({selectedCompanyIndex: reviewsList.length - 1})
    }
  }
  ...
  filteredReviewList = () => {
    ...
    const filteredList = reviewsList.filter(
      eachReviewItem =>
        selectedCompanyIndex === reviewsList.indexOf(eachReviewItem),
    )
    return filteredList
  }

  render() {
    const filteredList = this.filteredReviewList()

    return (
      <div className="bg-container">
        <h1 className="main-heading">Reviews</h1>
        <ul>
          {filteredList.map(eachReview => (
            <ReviewsCarousel
              eachReviewList={eachReview}
              key={eachReview.id}
              onClickLeftChange={this.onClickLeftChange}
              onClickRightChange={this.onClickRightChange}
            />
          ))}
        </ul>
      </div>
